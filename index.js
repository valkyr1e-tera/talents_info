module.exports = function talentsinfo(mod) {
  ...
  function msg() {
    ...
    mod.command.message(`LVL: <font color="#00FFFF">${level}</font>
    Used EP: <font color="#00FFFF">${usedPoints}/${totalPoints}</font>
    EXP: <font color="#00FFFF">${exp}</font>(NEXT: <font color="#00FFFF">${expTable[level+1] - Number(exp)}</font> EXP)
    DailyEXP: <font color="#00FFFF">${dailyExp}/${sdcap()}(${expCodicil})</font>`)
  }
